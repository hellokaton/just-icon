...
var ErrUserQuit = errors.New("user quit setup")
...
func SetupWizard() error {
	...
	fmt.Println("It looks like this is your first time using Just Icon. Let's set up your configuration.")
	fmt.Println()
...
func setupLanguage(configService *config.Service) error {
	...
	result, err := prompt.New().Ask("Select language:").Choose(languages)
	if err != nil {
		return err
	}
...
func setupBaseURL(configService *config.Service) error {
	...
	result, err := prompt.New().Ask(i18n.T("interactive_base_url_prompt")).Input(
		defaultURL,
		input.WithHelp(true),
	)
	if err != nil {
...
func setupAPIKey(configService *config.Service) error {
	...
	for {
		result, err := prompt.New().Ask(i18n.T("interactive_api_key_prompt")).Input(
			"",
			input.WithEchoMode(input.EchoPassword),
			input.WithValidateFunc(validateAPIKey),
		)
		if err != nil {
...
func setupOutputDirectory(configService *config.Service) error {
	...
	result, err := prompt.New().Ask(i18n.T("interactive_output_dir_prompt")).Input(
		defaultPath,
		input.WithHelp(true),
	)
	if err != nil {
